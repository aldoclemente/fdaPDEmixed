	...
	namespace apsc
	{
		...
		*/
		...
		class RotatingMatrix
		{
			...
			auto col(std::size_t j)const {return M_vec.col((oldestCol+j)%N);}
			//! Returns j-th column as an Eigen Vector
			decltype(auto) col(std::size_t j){return M_vec.col((oldestCol+j)%N);}
			...
			//! Returns the matrix as an Eigen Matrix Map
