...
template <class T>
class MixedFERegression_stopping_criterion : public FixedPoint::FixedPointIterator 
{
public:
...
{
	Eigen::SparseMatrix<Real, Eigen::RowMajor> res; // RowMajor ??? 
	...
	for (int k = 0; k < M_; ++k)
	{
		...
		// matrix.middleRows(i,q); rhs.block(i, j, p, q); 
	}
	res.pruned();
	return SpMat(res);
}
